...
interface CollectionState {
  ...
  actions: {
    ...
    createCollection: ({ name, parentId }: { name: string; parentId?: string }) => Promise<void>
...
const useCollectionStore = create<CollectionState>((set, get) => ({
  ...
  actions: {
      ...
      const { records: collectionItems } = await web5.dwn.records.query({
        message: {
          filter: {
            ...
            protocolPath: parentId ? 'collection/digitalDocument' : 'digitalDocument'
          }
        }
      })
      ...
      try {
        ...
        const { record } = await web5.dwn.records.create({
          ...
          message: {
            schema: 'https://schema.org/DigitalDocument',
            protocol: schemaOrgProtocolDefinition.protocol,
            protocolPath: get().collection?.id ? 'collection/digitalDocument' : 'digitalDocument',
    ...
    createCollection: async ({ name }: { name: string }) => {
      ...
      const { record } = await web5.dwn.records.create({
        ...
        message: {
          schema: 'https://schema.org/Collection',
          protocol: schemaOrgProtocolDefinition.protocol,
          protocolPath: get().collection ? 'collection/collection' : 'collection',
          dataFormat: 'application/json',
          published: true,
          parentId: get().collection?.id || undefined,
          contextId: get().collection?.contextId || undefined
    ...
    deleteItem: async (recordId: string) => {
      ...
      set((state) => ({
        collectionItems: state.collectionItems?.filter((item) => item.identifier !== recordId)
      }))
    ...
    handleSearch: (searchStr: string) => {
      ...
      if (searchStr === '') {
        ...
        const filteredCollectionItems = get().collectionItems?.filter((item) => {
