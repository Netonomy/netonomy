...
import { Person } from '@renderer/types/Person'
...
interface ProfileState {
  ...
  actions: {
    ...
    createProfile: ({ name, profileImg }: { name: string; profileImg: File }) => Promise<void>
...
const useProfileStore = create<ProfileState>((set) => ({
  ...
  actions: {
    ...
    createProfile: async ({ name, profileImg }: { name: string; profileImg: File }) => {
      try {
        const web5 = useWeb5Store.getState().web5
        ...
        const { record: profileImgRecord, status } = await web5.dwn.records.create({
          data: blob,
          message: {
            published: true
          }
    ...
    createConnection: async (did: string) => {
      ...
      try {
        ...
        if (updateRes?.status.code === 200) set({ profileRecord, profile: data })
      } catch (error) {
        console.error(error)
      }
    },
